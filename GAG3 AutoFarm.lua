-- ╔══════════════════════════════════════════════════╗
-- ║   HỒ QUỐC DEV — GROW A GARDEN 3  v3.1          ║
...
-- ══════════════════════════════════════════
--  AUTO STEAL — ban đêm
--  Ưu tiên best fruit, tele hái rồi về, lặp đến sáng
-- ══════════════════════════════════════════

-- Rarity priority (cao = quý hơn)
local RARITY_SCORE = {
...
-- Vị trí vườn của mình (cập nhật khi spawn)
local myFarmPos = nil
local function updateMyFarmPos()
    -- Tìm plot của mình trong workspace
    ...
    -- Fallback: vị trí hiện tại của mình
    local root = getRoot()
...
-- Thu thập tất cả prompt steal/harvest trên vườn người khác
local function collectStealTargets()
    local targets = {}
    for _, pp in ipairs(workspace:GetDescendants()) do
        if pp:IsA("ProximityPrompt") then
            local a = pp.ActionText:lower()
            local isTarget = a:find("steal") or a:find("harvest") or a:find("take") or a:find("pick")
            if isTarget then
                local part = getPart(pp.Parent)
                if part then
...
hT.Text="🌿 Hồ Quốc Dev — GAG3 Hub v1.0
