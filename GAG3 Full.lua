...
local RARITY_SCORE = {
    mythic=100, legendary=80, super=60, epic=40, rare=20, uncommon=10, common=1
}

local function getRarityScore(name)
    local n = name:lower()
    for rarity, score in pairs(RARITY_SCORE) do
        if n:find(rarity) then return score end
    end
    return 1
end
...
local function collectStealTargets()
    ...
    for _, pp in ipairs(workspace:GetDescendants()) do
        if pp:IsA("ProximityPrompt") then
            local a = pp.ActionText:lower()
            local isTarget = a:find("steal") or a:find("harvest") or a:find("take") 
                    or a:find("pick") or a:find("collect") or a:find("grab")
                    or a:find("fruit") or a:find("trái") or a:find("hái")
            if isTarget then
                local part = getPart(pp.Parent)
                if part then
                    ...
                    local isMine = owner and owner == LP.Name
                    if not isMine then
...
TL.Text="Hồ Quốc Dev  |  GAG3 Hub v1.0"
...
local function makePage(name)
    ...
    pg.BackgroundTransparency=1 pg.Visible=false pg.ZIndex=3
    ...
    ln.BackgroundColor3=C.divLine ln.BorderSizePixel=0 ln.ZIndex=4

    local c=Instance.new("Frame",pg)
    c.Size=UDim2.new(1,-8,1,-42) c.Position=UDim2.new(0,4,0,40)
    c.BackgroundTransparency=1 c.BorderSizePixel=0 c.ZIndex=4
...
local function mkToggle(parent,icon,label,key,order,cb)
    ...
    refresh() -- sync UI với CFG ngay khi tạo
...
shI.Text="Auto Buy: bat trong Automatically | Seed: "..CFG.SeedName.." | Gear: "..CFG.GearName
...
HQBtn.MouseButton1Click:Connect(function()
    if isDragging then return end  -- bỏ qua nếu đang drag
