...
local function collectStealTargets()
    ...
    for _, pp in ipairs(workspace:GetDescendants()) do
        if pp:IsA("ProximityPrompt") then
            local a = pp.ActionText:lower()
            -- Tìm prompt steal/take/grab của người khác
            local isTarget = a:find("steal") or a:find("take")
                or a:find("grab") or a:find("pick up")
            if isTarget then
                local part = getPart(pp.Parent)
                if part then
                    -- Bỏ qua vườn mình
                    ...
                    -- CHỈ steal từ Epic trở lên (score >= 400)
                    if score >= 400 then
    ...
    -- Sort: Mythic/Giant trước, rồi đến Legendary, Super, Epic
...
local function checkDefend()
    ...
    -- Scan player đang đứng trong vườn mình
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        local inMyPlot = isNearMyPlot(pRoot.Position, 25)

        -- Detect qua notification UI
        for _, v in ipairs(LP.PlayerGui:GetDescendants()) do
        ...
        -- Detect qua Highlight đỏ (hiệu ứng game gán cho kẻ trộm)
        local hl = p.Character:FindFirstChildWhichIsA("Highlight")
        if hl and hl:IsA("Highlight") then
            local col = hl.FillColor
            if col.R > 0.5 and col.G < 0.3 and col.B < 0.3 then
                inMyPlot = true
            end
        end
        ...
        if inMyPlot then
            if not thiefsDetected[p.Name] or tick() - thiefsDetected[p.Name] > 8 then
                ...
                markThief(p)
                task.spawn(function()
                    -- Liên tục đánh cho đến khi hắn rời đi
                    for i = 1, 5 do
                        if not p.Character then break end
                        local pr = p.Character:FindFirstChild("HumanoidRootPart")
                        if not pr then break end
                        if not isNearMyPlot(pr.Position, 30) then break end
                        autoAttackThief(p)
                        task.wait(0.5)
                    end
                end)
...
local function doBuySeed()
    -- Thử remote trước (không cần tele)
    if remoteBuySeed(CFG.SeedName) then return end

    -- Fallback: tele đến shop
    local root = getRoot()
    if not root then return end
    local farmPos = root.Position
    for _, pp in ipairs(workspace:GetDescendants()) do
        if pp:IsA("ProximityPrompt") then
            ...
            if (a:find("buy") or a:find("purchase")) and pName:find(CFG.SeedName:lower()) then
                local part = getPart(pp.Parent)
                if part then
                    root.CFrame = CFrame.new(part.Position + Vector3.new(0,3,0))
                    task.wait(0.05)
...
local function doBuyGear()
    -- Thử remote trước
    if remoteBuyGear(CFG.GearName) then return end
    ...
    for _, pp in ipairs(workspace:GetDescendants()) do
        if pp:IsA("ProximityPrompt") then
            local a = pp.ActionText:lower()
            local pName = pp.Parent.Name:lower()
            local targetName = CFG.GearName:lower()
            if (a:find("buy") or a:find("purchase")) and pName:find(targetName) then
                local part = getPart(pp.Parent)
                if part then
                    root.CFrame = CFrame.new(part.Position + Vector3.new(0,3,0))
                    task.wait(0.05)
...
local seedLabelF = Instance.new("Frame", aC)
seedLabelF.Size = UDim2.new(1,0,0,20)
seedLabelF.BackgroundTransparency = 1
seedLabelF.LayoutOrder = 3
local seedLbl = Instance.new("TextLabel", seedLabelF)
seedLbl.Size = UDim2.new(1,0,1,0)
seedLbl.BackgroundTransparency = 1
seedLbl.Text = "Seed: "..CFG.SeedName
seedLbl.TextColor3 = Color3.fromRGB(130,255,130)
seedLbl.TextSize = 10
seedLbl.Font = Enum.Font.GothamBold
seedLbl.TextXAlignment = Enum.TextXAlignment.Left

-- Grid seed buttons
local seedGrid = Instance.new("Frame", aC)
seedGrid.Size = UDim2.new(1,0,0,90)
seedGrid.BackgroundTransparency = 1
seedGrid.LayoutOrder = 4
local sgLL = Instance.new("UIGridLayout", seedGrid)
sgLL.CellSize = UDim2.new(0.5,-3,0,26)
sgLL.CellPadding = UDim2.new(0,3,0,3)

for _, sn in ipairs(seedNames) do
    local btn = Instance.new("TextButton", seedGrid)
    ...
    btn.MouseButton1Click:Connect(function()
        CFG.SeedName = sn
        seedLbl.Text = "Seed: "..sn
        -- highlight selected
        for _, c in ipairs(seedGrid:GetChildren()) do
            if c:IsA("TextButton") then
                c.BackgroundColor3 = c.Text == sn
                    and Color3.fromRGB(20,60,28)
                    or  Color3.fromRGB(22,24,36)
                c.TextColor3 = c.Text == sn and TEXON or DIM
            end
        end
    end)
    -- Highlight nếu là seed đang chọn
    if sn == CFG.SeedName then
        btn.BackgroundColor3 = Color3.fromRGB(20,60,28)
        btn.TextColor3 = TEXON
    end
end

mkSec(aC,"AUTO BUY GEAR",5)
mkToggle(aC,"","Auto Buy Gear","AutoBuyGear",6)

-- Gear dropdown buttons
local gearNames = {
    "Common Sprinkler","Uncommon Sprinkler","Rare Sprinkler",
    "Legendary Sprinkler","Super Sprinkler",
    "Common Watering Can","Super Watering Can",
    "Trowel","Gnome","Lantern","Basic Pot",
    "Wheelbarrow","Teleporter",
    "Jump Mushroom","Speed Mushroom","Shrink Mushroom",
    "Supersize Mushroom","Invisibility Mushroom",
}

local gearLabelF = Instance.new("Frame", aC)
gearLabelF.Size = UDim2.new(1,0,0,20)
gearLabelF.BackgroundTransparency = 1
gearLabelF.LayoutOrder = 7
local gearLbl = Instance.new("TextLabel", gearLabelF)
gearLbl.Size = UDim2.new(1,0,1,0)
gearLbl.BackgroundTransparency = 1
gearLbl.Text = "Gear: "..CFG.GearName
gearLbl.TextColor3 = Color3.fromRGB(130,255,130)
gearLbl.TextSize = 10
gearLbl.Font = Enum.Font.GothamBold
gearLbl.TextXAlignment = Enum.TextXAlignment.Left

local gearGrid = Instance.new("Frame", aC)
gearGrid.Size = UDim2.new(1,0,0,130)
gearGrid.BackgroundTransparency = 1
gearGrid.LayoutOrder = 8
local ggLL = Instance.new("UIGridLayout", gearGrid)
ggLL.CellSize = UDim2.new(0.5,-3,0,26)
ggLL.CellPadding = UDim2.new(0,3,0,3)

for _, gn in ipairs(gearNames) do
    local btn = Instance.new("TextButton", gearGrid)
    btn.BackgroundColor3 = Color3.fromRGB(22,24,36)
    btn.BorderSizePixel = 0
    btn.Text = gn
    btn.TextColor3 = DIM
    ...
    btn.MouseButton1Click:Connect(function()
        CFG.GearName = gn
        gearLbl.Text = "Gear: "..gn
        for _, c in ipairs(gearGrid:GetChildren()) do
            if c:IsA("TextButton") then
                c.BackgroundColor3 = c.Text == gn
                    and Color3.fromRGB(20,60,28)
                    or  Color3.fromRGB(22,24,36)
                c.TextColor3 = c.Text == gn and TEXON or DIM
            end
        end
    end)
    if gn == CFG.GearName then
        btn.BackgroundColor3 = Color3.fromRGB(20,60,28)
        btn.TextColor3 = TEXON
    end
end
